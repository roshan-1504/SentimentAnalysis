...
# Function to predict sentiment using SGD model
def predict_sentiment_sgd(input_text, model, vectorizer):
    ...
    # Predict using the saved SGD model
    prediction = model.predict(text_vectorized)
    
    # Return 'Positive' or 'Negative' based on the prediction
    return 'Positive' if prediction[0] == 1 else 'Negative'

# Define the predict endpoint for the SGD model
...
def predict():
    ...
    result = predict_sentiment_sgd(input_text, sgd_model, tfidf_vectorizer)
